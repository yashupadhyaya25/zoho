...
time_sheet_df = time_sheet_df.loc[:, ~time_sheet_df.columns.isin([
    ...
    'Hours(HH:MM)','Hrs','Min','Job Name','Project Name'
    ])]
time_sheet_df = time_sheet_df.pivot_table(index='Date', columns=['Employee Name'], values='Hours',aggfunc='sum')
time_sheet_df.fillna(0,inplace=True)
time_sheet_df.to_excel('./New Reports/Report_WO_Project_Task.xlsx')